...
\subsection{Discontinuity identity and branch-cut strength}

The analytic discontinuity of the radial solution across the cut is
characterised by the Casals--Ottewill identity~\cite{Casals2016}.
For the transmission-normalised upgoing solution
$\hat{R}^{\mathrm{up}} \equiv R^{\mathrm{up}}/C^{\mathrm{trans}}$
(which behaves as $\hat{R}^{\mathrm{up}} \sim r^{-2s-1}e^{+i\omega r^*}$ at infinity),
\begin{equation}
...
\subsection{Explicit formula for \texorpdfstring{$\Delta G^-$}{DeltaG-} and numerical evaluation}
...
\section{Branch cut strength for \texorpdfstring{$R^{\rm down}$}{Rdown}}
...
the analogue of $q$ defined in Eq.~\eqref{eq:q} for $R^{\rm up}$.
We derive $\tilde{q}$ below.
...
From DLMF \S13.2 (connection formula for Kummer's $U$),
