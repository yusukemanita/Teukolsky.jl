...
\begin{itemize}
  ...
  \item The curve $\psi_{\mathrm{num}} - \psiQNM$ (magenta, dash-dot) agrees
        closely with $\psi_{\mathrm{BC}}^-$ across the full $t > 0$ range,
...
\section{Branch Cut Strength}

\subsection{Discontinuity identity for \texorpdfstring{$R^{\mathrm{up}}$}{Rup}}

The analytic discontinuity across the negative imaginary axis can be related to
the MST amplitudes via the Casals--Ottewill identity \cite{Casals2016}:
\begin{align}
  \delta \hat{R}^{\mathrm{up}}
  \equiv \hat{R}^{\mathrm{up}}(\omega_R, r) - \hat{R}^{\mathrm{up}}(\omega_L, r)
  = -i\,q(\omega)\,R^{\mathrm{down}}(\omega, r),
  \label{eq:deltaRup}
\end{align}
where $\hat{R}^{\mathrm{up}} = R^{\mathrm{up}}/C^{\mathrm{trans}}$ is the
transmission-normalised upgoing solution ($\hat{R}^{\mathrm{up}} \sim r^{-2s-1}e^{+i\omega r^*}$
at infinity), $R^{\mathrm{down}}$ is the downgoing solution
($R^{\mathrm{down}} \sim r^{-2s-1}e^{-i\omega r^*}$), and $q(\omega)$ is the
\emph{branch-cut strength function}.
The sign convention differs by $-1$ from \cite{Casals2016} due to normalization choice.

\subsection{Branch-cut strength \texorpdfstring{$q(\omega)$}{q(omega)}}

In terms of MST parameters, $q$ is given by
\begin{align}
...
renormalised angular momentum computed from the continued-fraction equation.
This is implemented as \texttt{compute\_q} in \texttt{src/branch\_cut.jl}.

\subsection{Alternative formula for \texorpdfstring{$\Delta G^-$}{DeltaG-}}

Using~\eqref{eq:deltaRup}, the discontinuity $\Delta G^-$ of the Green's function
across the negative imaginary axis can be expressed directly in terms of
$q(\omega)$ and the asymptotic amplitudes~\cite{Casals2016}:
\begin{align}
  \Delta G^- = \frac{iq(\Bref)^2}{2i\omega\,\Binc\!\left(\Binc+iq\Bref\right)}.
  \label{eq:DGneg_q}
\end{align}
This formula involves only $q$, $\Binc$, and $\Bref$ evaluated at
$\omega_R = \delta - i\sigma$ on a single side of the cut,
\emph{with no subtraction}, so it avoids the catastrophic cancellation
that plagues the direct two-sided evaluation~\eqref{eq:DG_neg}.
As a result, \eqref{eq:DGneg_q} is numerically stable in standard
\texttt{Float64} precision across the full $\sigma$ range, as confirmed
by comparison with the BigFloat reference (see Fig.~\ref{fig:decomp}).

The agreement between $\psi_{\mathrm{BC}}^-$ (computed via
Eq.~\eqref{eq:DGneg_q} in \texttt{Float64}) and the residual
$\psi_{\mathrm{num}} - \psiQNM$ visible in Fig.~\ref{fig:decomp} demonstrates
that the late-time power-law tail is fully captured by the branch cut integral
even without extended precision arithmetic.
